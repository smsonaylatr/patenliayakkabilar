        ...
        return $table
            ...
            ->actions([
                ...
                Action::make('sendToPorego')
                    ->iconButton()
                    ->size('lg')
                    ->tooltip('Porego / Kargo Sistemine Gönder')
                    ->icon('heroicon-o-paper-airplane')
                    ->color('warning')
                    ->requiresConfirmation()
                    ->modalHeading('Porego Kargo Sistemine İlet')
                    ->modalDescription('Bu siparişi kargo etiketinin ve ürün detaylarının (SKU/Beden) güncellenmesi için Porego API\'sine iletmek istediğinize emin misiniz?')
                    ->modalSubmitActionLabel('Gönder')
                    ->action(function (Order $record): void {
                        $result = app(\App\Services\PoregoApiService::class)->sendOrder($record);
                        if ($result) {
                            \Filament\Notifications\Notification::make()
                                ->title('Porego\'ya İletildi')
                                ->body("Sipariş #{$record->order_number} kargo etiketinin basılması için Porego sistemine aktarıldı.")
                                ->success()
                                ->send();
                        } else {
                            \Filament\Notifications\Notification::make()
                                ->title('Porego Hatası')
                                ->body("Porego API aktarımı sırasında hata oluştu. Lütfen API ayarlarını kontrol edin.")
                                ->danger()
                                ->send();
                        }
                    }),
