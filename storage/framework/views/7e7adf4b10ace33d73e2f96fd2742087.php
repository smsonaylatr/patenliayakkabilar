                        ...
                        <div class="p-8">
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                                ...
                                <div class="mt-2 sm:mt-0">
                                    ...
                                    <?php elseif($order->payment_status === 'pending' && $order->payment_method === 'cash_on_delivery'): ?>
                                    ...
                                    <div class="mt-4 sm:mt-0 flex items-center gap-3 sm:gap-4 bg-gray-50/80 rounded-xl px-4 py-3 border border-gray-200">
                                        <div class="flex flex-col">
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <button onclick="navigator.clipboard.writeText('<?php echo e($order->cargo_tracking_code); ?>'); alert('Kopyalandı!');" aria-label="Kargo Kodunu Kopyala" class="text-gray-400 hover:text-gray-700 transition-colors focus:outline-none" title="Kodu Kopyala">
                            ...
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($order->status === 'delivered' && !in_array($order->id, $ratedOrderIds)): ?>
                                <?php
$__split = function ($name, $params = []) {
    return [$name, $params];
};
[$__name, $__params] = $__split('account.order-rating', ['order' => $order]);

$__keyOuter = $__key ?? null;

$__key = 'rating-'.$order->id;
$__componentSlots = [];

$__key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-2901921180-0', $__key);
...
unset($__split);
?>
                            <?php elseif($order->status === 'delivered' && in_array($order->id, $ratedOrderIds)): ?>
