<div class="mt-4 sm:mt-8" x-data="{ selectedId: <?php if ((object) ('selectedVariantId') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('selectedVariantId'->value()); ?>')<?php echo e('selectedVariantId'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('selectedVariantId'); ?>')<?php endif; ?>.live }">
    ...
    <div class="relative" x-data="{ 
        open: false,
        requiresSize: <?php echo e($requiresSize ? 'true' : 'false'); ?>,
        variants: [
            <?php $__currentLoopData = $product->variants; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $variant): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                { id: <?php echo e($variant->id); ?>, size: '<?php echo e(addslashes($variant->size)); ?>', color: '<?php echo e(addslashes(is_array($variant->color) ? implode(" / ", $variant->color) : ($variant->color ?? ""))); ?>', stock: <?php echo e($product->status ? $variant->stock : 0); ?> }<?php echo e(!$loop->last ? ',' : ''); ?>
        ...
        >
            <span x-text="selectedLabel" class="<?php echo e($allOutOfStock ? 'text-gray-400 font-medium' : 'text-gray-900'); ?>"></span>
