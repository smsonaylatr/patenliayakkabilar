...
<body class="antialiased text-gray-800" x-data="{ showModal: false, selectedProduct: null, phone: '', purpose: 'Çocuğum için 🧒🏻', isSubmitting: false, success: false, error: '' }">
    ...
    <div class="max-w-5xl mx-auto px-4 py-12">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-8">
            ...
            <div @click="selectedProduct = <?php echo e($product->id); ?>; showModal = true; success = false; error = '';" class="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 product-card cursor-pointer relative group flex flex-col">
                <div class="w-full overflow-hidden bg-gray-200 relative aspect-square">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($product->images->count() > 0): ?>
                        <img src="<?php echo e($product->images->first()->image_url); ?>" alt="<?php echo e($product->name); ?>" class="w-full h-full object-cover object-center transition-transform duration-500 group-hover:scale-105">
                    <?php else: ?>
                ...
                <div class="p-3 md:p-4 bg-white group-hover:bg-gray-50 transition-colors border-t border-gray-50 mt-auto">
                    <div class="w-full bg-white text-black font-semibold py-2 px-5 rounded-full flex items-center justify-center gap-2 shadow-sm border border-gray-200 transform group-hover:-translate-y-0.5 group-hover:shadow-md transition-all duration-300">
                        <span class="text-sm md:text-base">Seç</span>
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <div x-show="showModal" @click.away="showModal = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                    ...
                    <button @click="showModal = false" aria-label="Formu Kapat" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600">
...
<?php /**PATH C:\Users\Lenovo\Desktop\Projelerim\patenliayakkabilar.com\resources\views\landing\bilgi.blade.php ENDPATH**/ ?>