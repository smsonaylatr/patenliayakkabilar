    ...
    <div class="container mx-auto px-3 sm:px-4 max-w-md w-full">
        ...
        <div class="bg-white/90 backdrop-blur-2xl border border-white/60 rounded-2xl sm:rounded-3xl p-4 sm:p-7 md:p-8 shadow-[0_15px_35px_rgba(0,0,0,0.06)] sm:shadow-[0_30px_60px_rgba(0,0,0,0.08)] relative overflow-hidden">
            ...
            <div class="text-center mb-4 sm:mb-6 relative z-10">
                <h1 class="text-xl sm:text-2xl md:text-3xl font-black text-brand-dark mb-1 sm:mb-2 tracking-tight">Şifremi Unuttum</h1>
                <p class="text-gray-500 text-xs sm:text-sm">E-posta adresinizi girin, size şifre sıfırlama bağlantısı gönderelim.</p>
            </div>

            @if (session()->has('status') || $status)
                <div class="mb-4 bg-green-50 border border-green-200 text-green-700 rounded-xl p-3 sm:p-4 text-xs sm:text-sm font-medium relative z-10">
                    {{ session('status') ?? $status }}
                </div>
            @endif

            <form wire:submit="sendResetLink" class="space-y-3.5 sm:space-y-4 relative z-10">
                <!-- Email -->
                <div class="space-y-1">
            ...
            <div class="mt-5 sm:mt-7 relative z-10">
                <div class="relative">
                    ...
                    <div class="relative flex justify-center text-xs sm:text-sm">
                        <span class="px-2 bg-white/90 text-gray-500 font-medium rounded-full">veya</span>
                    </div>
                </div>

                <div class="mt-4 sm:mt-5 text-center">
                    <a href="{{ route('login') }}" class="text-brand-blue font-bold hover:underline text-xs sm:text-sm transition-colors">
                        Giriş Ekranına Dön
                    </a>
                </div>
            </div>
