    ...
    <div class="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-3">
        @foreach($product->variants as $variant)
            <button 
                ...
                type="button" 
                class="flex items-center justify-center border-2 py-2.5 w-full rounded-full text-sm font-medium transition-all duration-150 {{ $variant->stock <= 0 ? 'opacity-40 cursor-not-allowed line-through' : '' }}"
