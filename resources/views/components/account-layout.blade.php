        ...
        <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden min-h-[600px] flex flex-col md:flex-row">
            ...
            <div class="w-full md:w-72 bg-gray-50/50 border-r border-gray-100 p-8 flex flex-col">
                ...
                <nav class="flex-1 space-y-2">
                    <a href="{{ route('account.dashboard') }}" class="group flex items-center px-4 py-3 text-sm rounded-xl transition-all {{ request()->routeIs('account.dashboard') ? 'bg-black text-white shadow-md' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900' }}">
                    ...
                    <a href="{{ route('account.orders') }}" class="group flex items-center px-4 py-3 text-sm rounded-xl transition-all {{ request()->routeIs('account.orders') ? 'bg-black text-white shadow-md' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900' }}">
                        <svg class="w-5 h-5 mr-3 {{ request()->routeIs('account.orders') ? 'text-white' : 'text-gray-400 group-hover:text-gray-900' }} transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>
                        <span class="font-medium">Siparişlerim</span>
                    </a>

                    <a href="{{ route('account.profile') }}" class="group flex items-center px-4 py-3 text-sm rounded-xl transition-all {{ request()->routeIs('account.profile') ? 'bg-black text-white shadow-md' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900' }}">
