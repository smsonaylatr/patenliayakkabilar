...
<body class="antialiased text-gray-800" x-data="{ showModal: false, selectedProduct: null, phone: '', email: '', isSubmitting: false, success: false, error: '' }">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-8">
            ...
            <div @click="selectedProduct = {{ $product->id }}; showModal = true; success = false; error = '';" class="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 product-card cursor-pointer relative group">
                <div class="w-full overflow-hidden bg-gray-200 relative">
                    ...
                    <div class="absolute inset-0 transition-all duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100" style="background-color: rgba(0,0,0,0.15);">
    ...
    <div x-show="showModal" class="relative z-50" aria-labelledby="modal-title" role="dialog" aria-modal="true" x-cloak style="display: none;">
        <div x-show="showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-gray-900 bg-opacity-75 transition-opacity backdrop-blur-sm"></div>
